...
# INICIALIZANDO LA APLICACIÓN QUE TRABAJARA CON FLASK JWT------------------
jwt = JWTManager(app)
...
def registro():
    ...
    if user:
        return jsonify({"msg": "email ya esta en uso"}), 400
...
def login():
    ...
    if (one is None):
        ...
        if (check_password_hash(one.password, body["password"])):
            ...
            expiracion = datetime.timedelta(seconds=20000)
...
def recuperar_clave():
    # CONSULTA.
    body = request.get_json()
    # COMPARA EMAIL QUE SE INGRESA CON EL DE USER.
    existe_usuario = User.query.filter_by(email=body['email']).first()
    if (existe_usuario is None):
        raise APIException(
            "Ususario no existe.", status_code=401)
    else:
...
#     body = request.get_json()
#     # Esto compara el "email"  que llegó desde el body con los de la tabla User.
...
#     if (usuario is None):
#         raise APIException(
#             "Ususario no existe en la base de datos", status_code=401)
#     else:
